...
def get_tripo_sr():
    if "tripo_sr" not in _models:
        print("[LOAD] Installing TripoSR (first run, one-time)...", file=sys.stderr, flush=True)
        try:
            import subprocess
            subprocess.run(
            ...
            print(f"[LOAD] TripoSR pip install failed: {e}", file=sys.stderr, flush=True)
            _models["tripo_sr"] = None
            return _models["tripo_sr"]

        print("[LOAD] TripoSR installed, loading model...", file=sys.stderr, flush=True)
        try:
